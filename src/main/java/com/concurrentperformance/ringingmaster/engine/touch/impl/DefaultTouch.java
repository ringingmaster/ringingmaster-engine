...
public class DefaultTouch implements Touch {
	...
	//IMPORTANT NOTE: When adding items here, think about the clone method and immutability. Also toString
	private String name = "";
	private final NumberOfBells numberOfBells;
	private final Grid<TouchCell> cells;

	private Bell callingBell;
	...
	DefaultTouch(NumberOfBells numberOfBells, int columnCount, int rowCount) {
		this.numberOfBells = checkNotNull(numberOfBells, "numberOfBells must not be null");
		this.callingBell = numberOfBells.getTenor();
		this.cells = new DefaultGrid<TouchCell>(FACTORY, columnCount, rowCount);
		setTerminationMaxRows(SAFETY_VALVE_MAX_ROWS);
	}

	@Override
	public Touch clone() throws CloneNotSupportedException {
		DefaultTouch touchClone = new DefaultTouch(this.numberOfBells, this.cells.getColumnCount(), this.cells.getRowCount());
